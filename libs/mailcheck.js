var Mailcheck = {
    ...
    run: function(a) {
        a.domains = a.domains || Mailcheck.defaultDomains, a.secondLevelDomains = a.secondLevelDomains || Mailcheck.defaultSecondLevelDomains, 
        a.topLevelDomains = a.topLevelDomains || Mailcheck.defaultTopLevelDomains, a.distanceFunction = a.distanceFunction || Mailcheck.sift3Distance;
        var b = function(a) {
            return a;
        }, c = a.suggested || b, d = a.empty || b, e = Mailcheck.suggest(Mailcheck.encodeEmail(a.email), a.domains, a.secondLevelDomains, a.topLevelDomains, a.distanceFunction);
        return e ? c(e) : d();
    },
    suggest: function(a, b, c, d, e) {
        ...
        var h = this.findClosestDomain(f.secondLevelDomain, c, e, this.secondLevelThreshold), i = this.findClosestDomain(f.topLevelDomain, d, e, this.topLevelThreshold);
        if (f.domain) {
            var g = f.domain, j = !1;
            if (h && h != f.secondLevelDomain && (g = g.replace(f.secondLevelDomain, h), j = !0), 
            i && i != f.topLevelDomain && (g = g.replace(new RegExp(f.topLevelDomain + "$"), i), 
            j = !0), 1 == j) return {
                address: f.address,
                domain: g,
                full: f.address + "@" + g
...
$('#lemail').on('blur', function(event) {
  $(this).mailcheck({
    ...
    suggested: function(element, suggestion) {
      // callback code
      $('#suggestion').html("bedoel je <a href='#'>" + suggestion.full + "</a>?");
      $('#suggestion a').click(function(event) {
        $('#lemail').val(suggestion.full);
        $('#suggestion').html("");
        return false;
      });
